...
def saveFeatures(feature_extractor, features_file, labels_file, features_key, labels_key):
    ...
    * labels_key: name of the key for the hdf5 file to store the labels
    ...
    for folder, label in zip(folders, classes):
        ...
        for i in tqdm(range(flow.shape[0])):
...
def main():
    ...
    if do_training:
        ...
        for (train_index_falls, test_index_falls), (train_index_nofalls, test_index_nofalls) in zip(kf_falls.split(X_full[zeroes, ...]), kf_nofalls.split(X_full[ones, ...])):
            ...
            x = Dense(1, name='predictions', init='glorot_uniform')(extracted_features)
            ...
            if mini_batch_size == 0:
                history = classifier.fit(X,_y,  batch_size=X.shape[0], nb_epoch=epochs, shuffle='batch', class_weight=class_weight)
            else:
                history = classifier.fit(X,_y,  batch_size=mini_batch_size, nb_epoch=epochs, shuffle='batch', class_weight=class_weight)
...
"""                print("*****************MyPrediction*****************")
               prediction, truth = test_video(model, "fall3", 0)
               predicted = classifier.predict(np.asarray(prediction))
               print(predicted) """
